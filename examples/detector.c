...
void validate_detector_recall(char *cfgfile, char *weightfile)
{
    ...
    list *plist = get_paths("/home/gjw/darknet/scripts/Mydata/train.txt"); ///home/gjw/darknet/scripts/Mydata
            ...
            //resize_network(net, sized.w, sized.h);  
                ...
                if(outfile)  
                 {  
                    ...
                    save_image(im, "predictions");   //保存图像
    #ifdef OPENCV  
                ...
                int m = plist->size;  
                if(access("/home/gjw/darknet/voc/TestVoc",0)==-1)//进入失败  
                {  
		  printf("没有test目录，进入失败；新建目录\n");
                  if (mkdir("/home/gjw/darknet/voc/TestVoc",0777))//保存图像的路径  
                   {  
                     printf("创建目录失败\n");  
                   } 
		   else
	...
	///////
		char ch = '/';
		char *res = strrchr(path,ch)+1;

	///////
                sprintf(b,"/home/gjw/darknet/voc/TestVoc/%s",res);//保存图像的路径  

		printf("res = %s      b = %s\n\n",res,b);
                save_image(im, b);   //save_image(im, "predictions");  
